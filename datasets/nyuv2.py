...
class NYUv2Dataset(Dataset):
    ...
    def __init__(self, root_dir, mode, split=0.2, augment=False, input_size=(256, 256), in_domain='rgb', out_domain='seg'):
        ...
        self.mean = [100.02426444, 104.78370761, 122.54569849]
        self.std = [72.91530819, 70.39188641, 68.16926903]
    ...
    def __getitem__(self, idx):
        ...
        elif self.in_domain == 'seg':
          x = cv2.imread(in_path, cv2.IMREAD_GRAYSCALE)
          x = PILImage(x)

          if self.augment:
            x = resize_scale_nearest(x)
            i, j, h, w = transforms.RandomCrop.get_params(x, self.input_size)
            x = TF.crop(x, i, j, h, w)
        ...
        if self.out_domain == 'seg':
        ...
        elif self.out_domain == 'depth':
          y = io.imread(out_path)
          ...
          y = np.asarray(y, np.int16)

        elif self.out_domain == 'normal':
          y = io.imread(out_path)
          y = PILImage(y)
